...
def insert_messages(conn, messages, n):
    ...
    query += "COMMIT TRAN MessageInsert;"
    conn.execute_non_query(query)


# inserts n friend connections
def insert_friends(conn, n):
    query = "BEGIN TRAN FriendInsert;"
    ...
    query += "COMMIT TRAN FriendInsert;"
    conn.execute_non_query(query)
...
def insert_user_group(conn, n):
    ...
    for i in range(0, n):
        ...
        if group not in user_group_sets[user]:
            ...
            query += "INSERT INTO tblUSER_GROUPS([UserID],[GroupID]) VALUES(%s,%d);" % (user, group)
    query += "COMMIT TRAN UserGroupInsert;"
    conn.execute_non_query(query)
...
def insert_food_groups(conn, food_groups, descs):
    ...
    query += "COMMIT TRAN FoodGroupInsert;"
    conn.execute_non_query(query)


# insert n meal_items
def insert_meal_item(conn, food, n):
    query = "BEGIN TRAN MealItemInsert;"
    ...
    query += "COMMIT TRAN MealItemInsert;"
    conn.execute_non_query(query)


# insert n locations
def insert_location(conn, names, types, workouts, n):
    query = "BEGIN TRAN LocationInsert;"
    for i in range(0, n):
...
def insert_workout_type(conn, workouts, descs):
    ...
    query += "COMMIT TRAN WorkoutTypeInsert;"
    conn.execute_non_query(query)


# Inserts n workouts
def insert_workout(conn, start_year, end_year, comments, n):
    query = "BEGIN TRAN WorkoutInsert;"
    userids = get_users_ids(conn)
...
def insert_workout_band(conn, n):
    ...
    query += "COMMIT TRAN WorkoutBandInsert;"
    conn.execute_non_query(query)


# insert n badges
def insert_badge(conn, adverbs, adjectives, comments, n):
    query = "BEGIN TRAN BadgeInsert;"
    for i in range(0, n):
...
def insert_group_workout_type(conn, n):
    ...
    query += "COMMIT TRAN GroupWorkoutTypeInsert;"
    conn.execute_non_query(query)


# insert n Activity Group associations
def insert_group_activity(conn, n):
    query = "BEGIN TRAN GroupActivityInsert;"
    ...
    query += "COMMIT TRAN GroupActivityInsert;"
    conn.execute_non_query(query)